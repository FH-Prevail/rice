...
# ================================================================
#  Core packages
#  --skip-unavailable: skip any package not found without failing
#  NOTE: i3lock is intentionally NOT installed here — we build
#        i3lock-color from source which replaces it
# ================================================================
log "Installing core packages"
sudo dnf install -y --skip-unavailable \
  xorg-x11-xinit xorg-x11-server-Xorg xorg-x11-utils xorg-x11-server-utils \
  i3 i3status \
  xsettingsd \
  ...
  unzip wget git curl \
  @development-tools \
  automake autoconf libtool make pkgconf \
  giflib-devel \
...
#  Must remove system i3lock first if present, then build color ver
# ================================================================
log "Installing i3lock-color (replaces system i3lock)"
sudo dnf remove -y i3lock 2>/dev/null || true
...
if command -v i3lock >/dev/null 2>&1; then
  success "i3lock-color installed: $(i3lock --version 2>&1 | head -1)"
else
  info "WARNING: i3lock not found in PATH — betterlockscreen may not work"
fi
...
success "betterlockscreen installed"

# ================================================================
#  Betterlockscreen config — Catppuccin Mocha
#  All colors are 8-char rrggbbaa as required by i3lock-color
# ================================================================
...
bshl_color="f38ba8ff"
inside_color="11111bcc"
inside_ver_color="11111bcc"
inside_wrong_color="11111bcc"
...
info "Put your wallpapers in ~/Pictures/wallpapers/"
info "Name your main wallpaper: wall.jpg"
...
echo "  You need a wallpaper at ~/Pictures/wallpapers/wall.jpg"
echo ""
echo "    1) I already have ~/Pictures/wallpapers/wall.jpg — render it now"
echo "    2) Skip — I will run: betterlockscreen --update ~/Pictures/wallpapers/wall.jpg later"
echo ""
while true; do
...
if [[ "$LOCK_CHOICE" == "1" ]]; then
  if [[ -f "$HOME/Pictures/wallpapers/wall.jpg" ]]; then
    log "Pre-rendering lock screen..."
    betterlockscreen --update "$HOME/Pictures/wallpapers/wall.jpg"
    success "Lock screen rendered"
  else
    info "~/Pictures/wallpapers/wall.jpg not found — skipping render"
    info "Run: betterlockscreen --update ~/Pictures/wallpapers/wall.jpg"
  fi
else
  info "Skipped — run: betterlockscreen --update ~/Pictures/wallpapers/wall.jpg"
fi
...
echo "    2) Firefox — already on Fedora, just set as default"
echo "    3) Chrome  — Google Chrome"
echo "    4) Skip"
echo ""
while true; do
  read -rp "  Enter choice [1-4]: " BROWSER_CHOICE
  case "$BROWSER_CHOICE" in 1|2|3|4) break ;; *) echo "  Please enter 1-4." ;; esac
done
...
if [[ -n "$BROWSER_DESKTOP" ]]; then
  ...
  success "Default browser: $I3_BROWSER_CMD"
else
...
#  Uses [terminal.shell] format (new Alacritty ≥0.13)
# ================================================================
...
white   = "#a6adc8"
ALACRITTY
success "Alacritty configured with fish ($FISH_BIN)"
...
# ================================================================
#  Fish — config + Catppuccin colours
# ================================================================
...
TEAL=159; BLUE=111; LAVENDER=189; TEXT=253; OVERLAY0=240
...
if autorandr --save "$PROFILE" --force 2>/dev/null; then
  notify-send --icon=display --urgency=normal \
    "Monitor layout saved" "Profile: $PROFILE"
else
...
# ================================================================
#  i3 config — production ready, all errors fixed:
#    - smart_borders removed (deprecated in i3 4.25)
#    - $mod+Shift+E conflict fixed (ranger → $mod+Shift+F)
#    - volume/brightness notify-send quotes issue fixed (removed inline)
#    - wallpaper points to ~/Pictures/wallpapers/wall.jpg
# ================================================================
...
exec --no-startup-id xss-lock --transfer-sleep-lock -- /usr/local/bin/betterlockscreen -l blur
exec --no-startup-id /usr/libexec/polkit-gnome-authentication-agent-1

# ── exec_always (every reload) ───────────────────────────────────
...
bindsym $mod+Shift+f   exec $termfiles
...
# ── Multi-monitor (uncomment after: xrandr --query) ──────────────
...
printf '    Super+E          → thunar (GUI file manager)\n'
printf '    Super+Shift+F    → ranger (terminal file manager)\n'
printf '    Super+Tab        → window switcher\n'
printf '    Super+Shift+M    → arrange monitors\n'
printf '    Super+Shift+X    → lock screen\n'
printf '    Print            → screenshot fullscreen\n'
printf '    Super+Print      → screenshot region\n'
printf '    Super+Minus      → scratchpad toggle\n'
printf '    Fn+Brightness    → brightnessctl\n'
printf '    Fn+Volume        → pactl\n'
printf '    Alt+Shift        → switch keyboard US ↔ Persian\n'
echo ""
printf '\033[1;33m  Wallpaper:\033[0m\n'
printf '    Put wall.jpg in ~/Pictures/wallpapers/\n'
printf '    Then: betterlockscreen --update ~/Pictures/wallpapers/wall.jpg\n'
printf '    Then: Super+Shift+R to reload i3\n'
echo ""
