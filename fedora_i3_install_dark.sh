#!/usr/bin/env bash
# ================================================================
#  Fedora (i3 Spin recommended) — i3 Production Rice
...
sudo dnf install -y \
  xorg xinit i3 i3status xorg-x11-server-utils xsettingsd \
  rofi picom feh dex-autostart polkit-gnome lxappearance arandr autorandr \
  alacritty \
  thunar thunar-archive-plugin thunar-volman gvfs gvfs-fuse tumbler file-roller \
  ...
  tlp tlp-rdw \
  flameshot playerctl \
  ranger python3-pillow w3m w3m-img \
  fish \
  imagemagick bc xdpyinfo \
  unzip wget git curl @development-tools \
  ...
  libjpeg-turbo-devel \
  fontawesome-fonts google-noto-fonts-common google-noto-emoji-fonts jetbrains-mono-fonts \
...
echo "    2) Firefox — open-source, from Fedora repos"
...
case "$BROWSER_CHOICE" in
  ...
  2)
    ...
    # Firefox ships pre-installed on most Fedora spins.
    # Install/update via dnf to be sure.
...
# Fedora may ship with gdm (Workstation) or sddm (KDE spin).
# Switch to lightdm for our i3 setup.
for DM in gdm gdm3 sddm; do
  if systemctl is-enabled "$DM" >/dev/null 2>&1; then
    sudo systemctl disable "$DM" || true
    info "Disabled $DM — switching to LightDM"
  fi
done
...
sudo tee /etc/lightdm/lightdm-gtk.conf >/dev/null <<'GREETER'
[greeter]
theme-name=Arc-Dark
